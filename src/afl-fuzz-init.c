...
void pivot_inputs(afl_state_t *afl) {
  ...
  for (i = 0; i < afl->queued_items && likely(afl->queue_buf[i]); i++) {
    ...
    if (!strncmp(rsl, CASE_PREFIX, 3) &&
      ...
      s32 argv_fd = open(argv_fn, O_WRONLY | O_CREAT | O_EXCL, DEFAULT_PERMISSION);
...
void setup_dirs_fds(afl_state_t *afl) {
  ...
  if (!afl->in_place_resume) {
    ...
    fprintf(
        ...
        "argv_mut_exec, file_mut_exec, both_mut_exec, argv_mut_op, file_mut_op, both_mut_op\n");
