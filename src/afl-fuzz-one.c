...
u8 fuzz_one_original(afl_state_t *afl) {
  ...
  fprintf(afl->shrink_log, "FILEMUT: Selected %u, ARGV: %u, # of file for ARGV %u : %u, score : %u\n",
    afl->current_entry, argv_id, argv_id, afl->argv_to_queue_cnt[argv_id], perf_score);
...
#define FLIP_BIT(_ar, _b)                   \
  ...
  if (common_fuzz_stuff(afl, out_buf, len, argv_buf, argv_len)) { goto abandon_entry; }
...
skip_interest:
  ...
  for (i = 0; i <= (u32)len; ++i) {
    ...
    for (j = 0; j < afl->extras_cnt; ++j) {
      ...
      if (common_fuzz_stuff(afl, ex_tmp, len + afl->extras[j].len, argv_buf, argv_len)) {
...
skip_user_extras:
  ...
  for (i = 0; i <= (u32)len; ++i) {
    ...
    for (j = 0; j < afl->a_extras_cnt; ++j) {
      ...
      if (common_fuzz_stuff(afl, ex_tmp, len + afl->a_extras[j].len, argv_buf, argv_len)) {
  ...
  for (afl->stage_cur = 0; afl->stage_cur < afl->stage_max; ++afl->stage_cur) {
    ...
    if (afl->concat_style) {
      ...
      if (temp_len != afl->queue_cur->len) {
        ...
        afl->fsrv.is_file_changed = !!memcmp(orig_in + ARGV_MAX_SIZE, out_buf + ARGV_MAX_SIZE, temp_len - ARGV_MAX_SIZE);
      }
    }

    if (common_fuzz_stuff(afl, out_buf, temp_len, argv_buf, argv_len)) { goto abandon_entry; }
...
abandon_entry:
  ...
  if (total_run > 0) {
    fprintf(afl->shrink_log, "%llu/%u (%0.2f)%% found\n", new_hit_cnt - begin_hit_cnt,
      total_run, ((float) (new_hit_cnt - begin_hit_cnt)) / ((float) total_run) * 100.0);
