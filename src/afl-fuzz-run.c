...
void write_argv_file(afl_state_t *afl, u8 * argv, u32 argv_len) {
  ...
  for (index = 0; index < argv_len - 1; index++) {
    if (argv[index] == '@' && argv[index+1] == '@') {
      ...
      memcpy(argv_tmp + index + outfile_len, argv + index + 2, argv_len - index - 2);
  ...
  s32 fd = open(afl->fsrv.out_argv, O_WRONLY | O_CREAT | O_EXCL, DEFAULT_PERMISSION);
...
u8 trim_case(afl_state_t *afl, struct queue_entry *q, u8 *in_buf) {
  ...
  if (afl->concat_style) {
    if (orig_len <= ARGV_MAX_SIZE * TRIM_LIMIT) {
      return 0;
    }
  ...
  while (remove_len >= MAX(len_p2 / TRIM_END_STEPS, (u32)TRIM_MIN_BYTES)) {
    ...
    while (remove_pos < total_len) {
      ...
      if (afl->concat_style) {
        if ((total_len - trim_avail) <= ARGV_MAX_SIZE * TRIM_LIMIT) {
          break;
        }
  ...
  if (needs_write) {
    ...
    if (unlikely(afl->concat_style)) {
      queue_testcase_retake_mem(afl, q, in_buf, q->len + ARGV_MAX_SIZE, orig_len);
    } else {
...
common_fuzz_stuff(afl_state_t *afl, u8 *out_buf, u32 len, u8 * argv, u32 argv_len) {
  ...
  if (afl->concat_style) {
    ...
    if (unlikely(write_to_testcase(afl, (void **)&tmp, len - ARGV_MAX_SIZE, 0) == 0)) {
      return 0;
    } 
  } else {
  ...
  if (afl->concat_style) {
    afl->queued_discovered += save_if_interesting(afl, out_buf + ARGV_MAX_SIZE, len - ARGV_MAX_SIZE, fault, out_buf, ARGV_MAX_SIZE);
  } else {
    afl->queued_discovered += save_if_interesting(afl, out_buf, len, fault, argv, argv_len);
  }
