  ...
  if (likely(afl->schedule < RARE)) {
    ...
    for (i = 0; i < n; i++) {
      ...
      if (afl->mut_file_only && q->disabled_file_mut) {
        ...
        double weight = compute_weight(afl, q, avg_exec_us, avg_bitmap_size, avg_top_size);
...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det, u8 * argv, u32 argv_len, u8 * argv_fn) {
    ...
    if (afl->fsrv.trace_bits[i]) {
      ...
      if (afl->top_rated[i]) {
        ...
        if (unlikely(afl->schedule >= FAST && afl->schedule <= RARE))
          ...
          top_rated_fuzz_p2 = afl->top_rated[i]->fuzz_level_file + afl->top_rated[i]->fuzz_level_argv;
...
u32 calculate_score(afl_state_t *afl, struct queue_entry *q) {
  ...
  switch (afl->schedule) {
    ...
    case QUAD:
      factor =
          q->fuzz_level_file * q->fuzz_level_file / (afl->n_fuzz[q->n_fuzz_entry] + 1);
      break;
    ...
    while (unlikely(
      ...
      if (unlikely(afl->q_testcase_cache_size + len >=
        ...
        afl->q_testcase_cache = ck_realloc(
            ...
            (afl->q_testcase_max_cache_entries + 1) * sizeof(struct queue_entry *));
...
void log_queue_entry(afl_state_t * afl) {
  ...
  if (fp == NULL) {
    PFATAL("Unable to open '%s'", file_fn);
  }

  fprintf(fp, "queue_id:len:argv_id:argv_len:fuzz_level_argv:fuzz_level_file:depth\n");
