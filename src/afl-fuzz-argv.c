...
static u32        split_argv(u8 *argv, u32 argv_len, u8 **argv_word_buf);
...
void select_argv(afl_state_t *afl) {
  ...
  if (afl->select_random_argv) {
    memset(selected_argvs_bits, 0, num_argvs * sizeof(u8));
    ...
    for (idx1 = 0; idx1 < num_selected; idx1++) {
      ...
      while (exists) {
        exists = 0;
        rand_idx = rand_below(afl, num_argvs);
        for (idx2 = 0; idx2 < idx1; idx2++) {
          if (selected_argvs[idx2] == rand_idx) {
            exists = 1;
            break;
          }
        }
      }
      selected_argvs[idx1] = rand_idx;
      selected_argvs_bits[rand_idx] = 1;
    }
  }
...
void read_old_argv_dir(afl_state_t *afl) {
  ...
  if (!plot_f) { PFATAL("Unable to open '%s'", plot_fn); }
  ck_free(plot_fn);
  ...
  while (fgets(buffer, 512, plot_f)) {
    ...
    if (num_input_to_read != 0) { break; }
  }

  fprintf(afl->debug_file, "# of input to read : %u\n", num_input_to_read);

  if (unlikely(num_input_to_read == 0)) { PFATAL("Can't get old inputs"); }
  ...
  if (!queue_log_f) { PFATAL("Unable to open '%s'", queue_log_fn); }
  ...
  while (fgets(buffer, 512, queue_log_f)) {
    ...
    if (queue_idx >= num_input_to_read) { break; }
  ...
  while (idx1 > 0) {
    ...
    if (argv_fd < 0) { PFATAL("Unable to open '%s'", argv_fn); }
...
static u32 **leven_matrix;
static u32   levenshtein_distance(u8 *a, u32 a_len, u8 *b, u32 b_len) {
  leven_matrix[0][0] = 0;

  u32 idx1, idx2;
  for (idx1 = 1; idx1 <= b_len; idx1++) {
    leven_matrix[idx1][0] = leven_matrix[idx1 - 1][0] + 1;
  }

  for (idx1 = 1; idx1 <= a_len; idx1++) {
    leven_matrix[0][idx1] = leven_matrix[0][idx1 - 1] + 1;
  }

  for (idx1 = 1; idx1 <= b_len; idx1++) {
    for (idx2 = 1; idx2 <= a_len; idx2++) {
      u32 dist1 = leven_matrix[idx1 - 1][idx2] + 1;
      u32 dist2 = leven_matrix[idx1][idx2 - 1] + 1;
      u32 dist3 =
          leven_matrix[idx1 - 1][idx2 - 1] + (a[idx2 - 1] != b[idx1 - 1]);
      u32 min = dist1;
      if (dist2 < min) min = dist2;
      if (dist3 < min) min = dist3;
      leven_matrix[idx1][idx2] = min;
    }
  }

  return leven_matrix[b_len][a_len];
}

void select_argv_text(afl_state_t *afl) {
  u32 **argv_to_queue = afl->argv_to_queue;
  u32  *num_argv_to_queue = afl->argv_to_queue_cnt;
  u32   num_argvs = afl->num_argvs;
  ...
  // Put random 2 argvs into selected_argvs
  u8  *selected_argvs_bits = (u8 *)calloc(num_argvs, sizeof(u8));
  u32 *selected_argvs = (u32 *)malloc(sizeof(u32) * num_argvs);
  u32  rand_1 = rand_below(afl, num_argvs);
  u32  rand_2 = rand_below(afl, num_argvs);
  ...
  float **argv_dist = (float **)malloc(sizeof(float *) * num_argvs);
  for (idx1 = 0; idx1 < num_argvs; idx1++) {
    argv_dist[idx1] = (float *)calloc(num_argvs, sizeof(float));
  }

  float dist_total_sum = 0.0;
  float max_dist = 1.0;

  if (afl->text_argv_selection == 1 || afl->text_argv_selection == 2) {
    // levenstein distance (sorted)

    u8 **argvs = (u8 **)malloc(sizeof(u8 *) * num_argvs);
    u32 *argv_lens = (u32 *)malloc(sizeof(u32) * num_argvs);

    for (idx1 = 0; idx1 < num_argvs; idx1++) {
      struct queue_entry *q = afl->queue_buf[argv_to_queue[idx1][0]];
      argvs[idx1] = q->argv;
      argv_lens[idx1] = q->argv_len;
    }

    if (afl->text_argv_selection == 2) {
      u8 **argv_buf = malloc(sizeof(u8 *) * MAX_ARGV_WORDS);
      u8 **sorted_argv_buf = malloc(sizeof(u8 *) * MAX_ARGV_WORDS);

      for (idx1 = 0; idx1 < num_argvs; idx1++) {
        struct queue_entry *q = afl->queue_buf[argv_to_queue[idx1][0]];

        u32 num_argv_word = split_argv(q->argv, q->argv_len, argv_buf);

        u32 sorted_idx = 0;

        while (sorted_idx < num_argv_word) {
          u8 *min_str = NULL;
          u32 min_idx = (u32)-1;
          for (idx2 = 0; idx2 < num_argv_word; idx2++) {
            if (argv_buf[idx2] == NULL) { continue; }

            if (min_str == NULL) {
              min_str = argv_buf[idx2];
              min_idx = idx2;
              continue;
            }

            if (strcmp(argv_buf[idx2], min_str) < 0) {
              min_str = argv_buf[idx2];
              min_idx = idx2;
            }
          }

          sorted_argv_buf[sorted_idx++] = min_str;
          argv_buf[min_idx] = NULL;
        }

        u8 *sorted_argv_strbuf = malloc(sizeof(u8) * ARGV_MAX_SIZE);
        u32 argv_len = 0;
        for (idx2 = 0; idx2 < sorted_idx; idx2++) {
          u32 word_len = strlen(sorted_argv_buf[idx2]) + 1;
          memcpy(sorted_argv_strbuf + argv_len, sorted_argv_buf[idx2],
                 word_len);
          argv_len += word_len;
        }
        sorted_argv_strbuf[argv_len++] = 0;

        argvs[idx1] = sorted_argv_strbuf;
        argv_lens[idx1] = argv_len;

        for (idx2 = 0; idx2 < sorted_idx; idx2++) {
          free(sorted_argv_buf[idx2]);
        }
      }

      free(argv_buf);
      free(sorted_argv_buf);
    }
    ...
    leven_matrix = (u32 **)malloc(sizeof(u32 *) * (ARGV_MAX_SIZE + 1));
    for (idx1 = 0; idx1 < ARGV_MAX_SIZE + 1; idx1++) {
      leven_matrix[idx1] = (u32 *)malloc(sizeof(u32) * (ARGV_MAX_SIZE + 1));
    }

    u32 argv_1, argv_2;
    for (argv_1 = 0; argv_1 < num_argvs - 1; argv_1++) {
      afl->stage_cur = argv_1;

      u8 *argv1_str = argvs[argv_1];
      u32 argv1_len = argv_lens[argv_1];

      for (argv_2 = argv_1 + 1; argv_2 < num_argvs; argv_2++) {
        for (idx1 = 0; idx1 < ARGV_MAX_SIZE + 1; idx1++) {
          memset(leven_matrix[idx1], 0, sizeof(u32) * (ARGV_MAX_SIZE + 1));
        }

        u8 *argv2_str = argvs[argv_2];
        u32 argv2_len = argv_lens[argv_2];

        u32 dist =
            levenshtein_distance(argv1_str, argv1_len, argv2_str, argv2_len);

        argv_dist[argv_1][argv_2] = argv_dist[argv_2][argv_1] = (float)dist;

        if (dist > max_dist) { max_dist = dist; }

        dist_total_sum += 2.0 * dist;
      }
    ...
    for (idx1 = 0; idx1 < ARGV_MAX_SIZE + 1; idx1++) {
      free(leven_matrix[idx1]);
    }
    free(leven_matrix);

  } else if (afl->text_argv_selection == 3) {
    // set jaccard

    float dist_total_sum = 0.0;

    u8 ***argv_words = (u8 ***)malloc(sizeof(u8 **) * num_argvs);
    u32  *num_argv_words = (u32 *)malloc(sizeof(u32) * num_argvs);

    for (idx1 = 0; idx1 < num_argvs; idx1++) {
      struct queue_entry *q = afl->queue_buf[argv_to_queue[idx1][0]];

      argv_words[idx1] = (u8 **)malloc(sizeof(u8 *) * MAX_ARGV_WORDS);
      u32 num_argv_word = split_argv(q->argv, q->argv_len, argv_words[idx1]);

      // remove duplicate
      u32 idx3;
      for (idx2 = num_argv_word - 1; idx2 > 1; idx2--) {
        for (idx3 = 0; idx3 < idx2; idx3++) {
          if (strcmp(argv_words[idx1][idx2], argv_words[idx1][idx3]) == 0) {
            free(argv_words[idx1][idx2]);
            memmove(argv_words[idx1] + idx2, argv_words[idx1] + idx2 + 1,
                    sizeof(u8 *) * (num_argv_word - idx2 - 1));
            num_argv_word--;
            break;
          }
        }
      ...
      num_argv_words[idx1] = num_argv_word;
    }

    u8 **argv_buf = malloc(sizeof(u8 *) * 2 * MAX_ARGV_WORDS);
    u32  num_total_argv;

    for (idx1 = 0; idx1 < num_argvs - 1; idx1++) {
      u32  num_argv1_words = num_argv_words[idx1];
      u8 **argv1_words = argv_words[idx1];

      memcpy(argv_buf, argv1_words, sizeof(u8 *) * num_argv1_words);

      num_total_argv = num_argv1_words;

      for (idx2 = idx1 + 1; idx2 < num_argvs; idx2++) {
        u32  num_argv2_words = num_argv_words[idx2];
        u8 **argv2_words = argv_words[idx2];

        u32 num_common_words = 0;
        num_total_argv = num_argv1_words;

        u32 idx3, idx4;
        for (idx3 = 0; idx3 < num_argv2_words; idx3++) {
          u8 is_common = 0;
          for (idx4 = 0; idx4 < num_argv1_words; idx4++) {
            if (!strcmp(argv2_words[idx3], argv1_words[idx4])) {
              is_common = 1;
              break;
            }
          }
          ...
          if (is_common) {
            ...
            argv_buf[num_total_argv++] = argv2_words[idx3];
          }
        }

        float jaccard_dist =
            (num_total_argv - num_common_words) / (float)num_total_argv;

        argv_dist[idx1][idx2] = argv_dist[idx2][idx1] = jaccard_dist;

        dist_total_sum += 2.0 * jaccard_dist;
      }
    }

    for (idx1 = 0; idx1 < num_argvs; idx1++) {
      for (idx2 = 0; idx2 < num_argv_words[idx1]; idx2++) {
        free(argv_words[idx1][idx2]);
      }
      free(argv_words[idx1]);
    }
    free(argv_words);
    free(num_argv_words);
    free(argv_buf);
  ...
  float threshold =
      dist_total_sum / (((float)num_argvs) * ((float)(num_argvs - 1)));
  ...
  for (idx1 = 0; idx1 < num_argvs; idx1++) {
    ...
    bool is_close = false;
    for (idx2 = 0; idx2 < num_selected; idx2++) {
      if (argv_dist[idx1][selected_argvs[idx2]] < threshold) {
        is_close = true;
        break;
      }
    }
  ...
  u8 file_fn[PATH_MAX];
  snprintf(file_fn, PATH_MAX, "%s/power/argv_dists", afl->out_dir);
  FILE *fp = fopen(file_fn, "w");
  if (fp == NULL) { PFATAL("Unable to create '%s'", file_fn); }
  fprintf(fp, "Avg : %0.4f\n", threshold);
  ...
  for (idx1 = 0; idx1 < num_argvs; idx1++) {
    for (idx2 = 0; idx2 < num_argvs; idx2++) {
      u32 hist_idx = (u32)(argv_dist[idx1][idx2] * 20.0 / max_dist);
      if (unlikely(hist_idx >= 21)) {
        fprintf(afl->debug_file, "argv_dist[%u][%u] = %0.4f", idx1, idx2,
                argv_dist[idx1][idx2]);
        fprintf(afl->debug_file, "hist_idx = %u, max_dist = %0.4f", hist_idx,
                max_dist);
        continue;
      }
      histogram[(u32)(argv_dist[idx1][idx2] * 20.0 / max_dist)] += 1;
    }
  }
...
static u32 split_argv(u8 *argv, u32 argv_len, u8 **argv_word_buf) {
  ...
  for (idx1 = 0; idx1 < argv_len - 1; idx1++) {
    if (argv[idx1] == 0) {
      u32 word_len = idx1 - (cur_ptr - argv);
      argv_word_buf[argv_word_idx] = malloc(word_len + 1);
      memcpy(argv_word_buf[argv_word_idx++], cur_ptr, word_len + 1);
      cur_ptr = argv + idx1 + 1;

      if (unlikely(argv_word_idx >= MAX_ARGV_WORDS)) { break; }
    }
  }

  return argv_word_idx;
}
...
void shrink_corpus(afl_state_t *afl) {
  ...
  for (idx1 = 0; idx1 < afl->num_argvs; idx1++) {
    ...
    if (num_queue == 1) { num_onefile_argv++; }
  ...
  for (idx1 = 0; idx1 < afl->num_unique_files; idx1++) {
    ...
    if (num_queue == 1) { num_oneargv_files++; }
...
void shrink3(afl_state_t *afl) {
  ...
  u32                 idx1, idx2, queue_idx, argv_idx;
  struct queue_entry *q;
  ...
  if (afl->interleaving == 3) {
    if (num_active_zero_argvs < NUM_SEL_ZERO_ARGVS) {
      memcpy(selected_argvs, active_zero_argvs,
             sizeof(u32) * num_active_zero_argvs);
    } else {
      u8 *selected_bits = (u8 *)calloc(num_active_zero_argvs, sizeof(u8));

      while (num_selected <
             NUM_SEL_ZERO_ARGVS) {  // (num_active_zero_argvs *
                                    // REMOVE_ZERO_ARGVS_RATIO)) {
        idx1 = rand_below(afl, num_active_zero_argvs);
        if (selected_bits[idx1]) { continue; }

        selected_bits[idx1] = 1;
        selected_argvs[num_selected++] = active_zero_argvs[idx1];
      }

      free(selected_bits);
    }
  } else if (afl->interleaving == 6) {
    u32 *rank_scores = (u32 *)calloc(num_active_zero_argvs, sizeof(u32));

    for (idx1 = 0; idx1 < num_active_zero_argvs; idx1++) {
      argv_idx = active_zero_argvs[idx1];
      q = afl->queue_buf[afl->argv_to_queue[argv_idx][0]];

      u32 score = calculate_score(afl, q);

      for (idx2 = 0; idx2 < idx1; idx2++) {
        if (score >= rank_scores[idx2]) {
          memmove(rank_scores + idx2 + 1, rank_scores + idx2,
                  sizeof(u32) * (idx1 - idx2));
          memmove(selected_argvs + idx2 + 1, selected_argvs + idx2,
                  sizeof(u32) * (idx1 - idx2));

          rank_scores[idx2] = score;
          selected_argvs[idx2] = argv_idx;
          break;
        }
      }
    ...
    if (num_active_zero_argvs > NUM_SEL_ZERO_ARGVS) {
      num_selected = NUM_SEL_ZERO_ARGVS;
    } else {
      num_selected = num_active_zero_argvs;
    }
  ...
  for (idx1 = 0; idx1 < afl->queued_items; idx1++) {
    if (selected_queue_bits[idx1] == 1) { continue; }
