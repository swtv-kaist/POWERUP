...
save_if_interesting(afl_state_t *afl, void *mem, u32 len, u8 fault, u8 * argv, u32 argv_len) {
  ...
  save_to_queue:
    ...
    u8 * argv_fn = alloc_printf("%s/queue_argvs/id:%06u", afl->out_dir, afl->queued_items);
    ...
    //fprintf(afl->func_cov_log, "%u,%u,%u\n", afl->queued_items - 1, afl->queue_top->argv_id, afl->queue_top->file_id);
    ...
    if (afl->interleaving == 0 && afl->concat_style == 0) {
      u32 idx1, idx2;
  ...
  switch (fault) {
    ...
    case FSRV_RUN_TMOUT:
     ...
     snprintf(argv_fn, PATH_MAX, "%s/hangs_argvs/id:%06llu", afl->out_dir, afl->saved_hangs);
    ...
    keep_as_crash:
      ...
      snprintf(argv_fn, PATH_MAX, "%s/crashes_argvs/id:%06llu", afl->out_dir, afl->saved_crashes);
      ++afl->saved_crashes;
#ifdef INTROSPECTION
      if (afl->custom_mutators_count && afl->current_custom_fuzz) {
