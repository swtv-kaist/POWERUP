...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while (
      (opt = getopt(
           argc, argv,
           "+Aa:b:B:c:C:dD:e:E:f:F:g:G:hi:I:j:K:l:L:m:M:nNOo:P:p:Qq:Rrs:S:t:T:UV:WXx:YZ")) >
      0) {
    ...
    switch (opt) {
      ...
      case 'q':
        afl->use_comb_argv_mut_only = atoi(optarg);
      ...
      case 'K':
        ...
        //2 - 1 + func cov based cmd shrink
        //3 - 1 + remove some random zero commands
        //4 - 2 + shrink at the end of both cmds/files mutations
        //5 - 2 + shrink file, too
        //6 - 1 + rank based on afl-score
        break;
      ...
      case 'r':
        PFATAL("select_random_argv is disabled now");
        afl->select_random_argv = 1;
        break;
      
      case 'j':
        afl->text_argv_selection = atoi(optarg);
        //1 -> just edit-distance 
        //2 -> sort thatn edit-distance
        //3 -> jaccard
        break;
      ...
      case 'P': {
        afl->argv_random_mut_prob = atoi(optarg);
        break;
      }
      ...
      case 'C':                                              
        ...
        afl->concat_style = atoi(optarg);
        //1 -> concat
        //2 -> 1 + comb argv mutation
        //3 -> 1 + afl++ built-in argv mutation
        break;
  ...
  afl->func_exec_map = ck_realloc(afl->func_exec_map, FUNC_MAP_SIZE * sizeof(u32 *));
  ...
  fprintf(afl->debug_file, "# of useless : %u/%u\n", afl->useless_at_start, afl->queued_items);
  ...
  while (likely(!afl->stop_soon)) {
    ...
    do {
      ...
      if (afl->mut_file_only) {
        ...
        if (afl->concat_style == 2) {
          skipped_fuzz = fuzz_one_concat_comb(afl);
        } else if (afl->concat_style == 3) {
          skipped_fuzz = fuzz_one_concat_argv(afl);
        } else if (afl->use_comb_argv_mut_only == 1) {
          skipped_fuzz = fuzz_one_comb_argv(afl);
        } else if (afl->use_comb_argv_mut_only == 2) {
          skipped_fuzz = fuzz_one_argv(afl);
        } else {
          if (rand_below(afl, 100) < afl->argv_random_mut_prob) {
            skipped_fuzz = fuzz_one_argv(afl);
          } else {
            skipped_fuzz = fuzz_one_comb_argv(afl);
          }
    ...
    if (afl->interleaving) {
      ...
      if (unlikely(afl->mut_file_only && (time_passed > afl->interleaving_timeout[1]))) {
        //Mutate argv from now
        afl->last_stage_change_time = get_cur_time();
        afl->mut_file_only = 0;
        afl->reinit_table = 1;
        write_shrink_log(afl);
        if (afl->interleaving == 2 || afl->interleaving == 4) {
          shrink_corpus(afl);
          update_all_bitmap_score(afl);
        } else if (afl->interleaving == 3 || afl->interleaving == 6) {
          shrink3(afl);
          update_all_bitmap_score(afl);
        } else if (afl->interleaving == 5) {
          shrink_corpus2(afl);
          update_all_bitmap_score(afl);
        }
    ...
    } else if (afl->concat_style == 1) {

    } else if (afl->concat_style == 2 || afl->concat_style == 3) {
      afl->fair_mut_count++;
      if (afl->mut_file_only && (afl->fair_mut_count >= FAIR_FILE_MUT_TIME)) {
        afl->mut_file_only = 0;
        afl->fair_mut_count = 0;
      } else if (!afl->mut_file_only && (afl->fair_mut_count >= FAIR_CMD_MUT_TIME)) {
        afl->mut_file_only = 1;
        afl->fair_mut_count = 0;
      }
    } else {
    ...
    if (unlikely(afl->select_argv == 1)) {
      if (afl->text_argv_selection) {
        select_argv_text(afl);
      } else {
        select_argv(afl);
      }
      afl->select_argv = 2;
