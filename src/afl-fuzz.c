...
nyx_plugin_handler_t *afl_load_libnyx_plugin(u8 *libnyx_binary) {
  ...
  if (plugin->nyx_new_parent == NULL) {
    goto fail;
  }

  plugin->nyx_new_child = dlsym(handle, "nyx_new_child");
  if (plugin->nyx_new_child == NULL) {
    goto fail;
  }
...
int main(int argc, char **argv_orig, char **envp) {
  ...
  while ((opt = getopt(argc, argv,
                       "+Aa:b:B:c:C:dD:e:E:f:F:g:G:hi:I:j:K:l:L:m:M:nNOo:P:p:"
                       "Qq:Rrs:S:t:T:UV:WXx:YZ")) > 0) {
    switch (opt) {
      case 'a':
  ...
  if (get_afl_env("AFL_DISABLE_TRIM")) {
    afl->disable_trim = 1;
  }
  ...
  while (likely(!afl->stop_soon)) {
    ...
    if (afl->interleaving) {
      ...
      if (afl->mut_file_only && (time_passed > afl->interleaving_timeout[1])) {
      ...
      if (afl->mut_file_only && (afl->fair_mut_count >= FAIR_FILE_MUT_TIME)) {
        ...
        afl->fair_mut_count = 0;
                 ...
                 (afl->fair_mut_count >= FAIR_CMD_MUT_TIME)) {
        afl->mut_file_only = 1;
        afl->fair_mut_count = 0;
      }
    } else {
